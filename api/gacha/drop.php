...
try{
  ...
  // ユーザーが存在しているかチェック
  if( $buff === false ){
    utility::sendResponse(false, 'Not Found User');
    exit(1);
  }

  // 残高が足りているかチェック
  if( $buff['money'] < GACHA_PRICE ){
    utility::sendResponse(false, 'The balance is not enough');
    exit(1);
  }
...
// データが0件
if( $buff === false ){
  utility::sendResponse(false, 'System Error');
}
// データを正常に取得
else{
  utility::sendResponse(true, $chara);
}
